...
export default function VPDGraph({ data, lineColor, fillGraph, selectedDate, title, makeGraph, tension }){
  ...
  if(data == null){
    ...
    if (columns.length >= 3) {
      ...
      const tempValue = columns[1]; 
      const vpdValue = calculateVPD(tempValue, secondColumnValue); 

      firstColumnValues.push(firstColumnValue);
      secondColumnValues.push(secondColumnValue);
      vpdValues.push(vpdValue); 
    }
    }


    function calculateVPD(tempValue, relativeHumidity) {
      const a = parseFloat(relativeHumidity); 
      const b = parseFloat(tempValue); 
      const SVP = 0.61078 * Math.exp((17.625 * b) / (b + 237.3));
      const VPD = SVP * (1 - a / 100);
