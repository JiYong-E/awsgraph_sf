...
import DLI from "../api/components/DLI";
import TempMAM from "../api/components/TempMAM";
import GDD from "../api/components/GDD";
...
export default function YouWant() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
              ...
              const res = await fetch(`/api/components/ReadUtilData?startIdx=${startIdx}&endIdx=${endIdx}`);
  ...
  const upperCaseId = id === 'DLI' ? 'DLI' : id === 'GDD' ? 'GDD' : id === 'avgtemp' ? 'Average Temperature' : 'Invalid ID';
  ...
  const handleDecrementcompression = () => {
    ...
    return (
        ...
        {id === 'DLI' && <DLI data={data} lineColor={color} startIdx={startIdx} endIdx={endIdx} fillGraph={fillGraph} tension={tension} makeGraph={makeGraph} selectedDate={selectedDate} title={title} />}
        {id === 'GDD' && <GDD data={data} lineColor={color} startIdx={startIdx} endIdx={endIdx} fillGraph={fillGraph} tension={tension} makeGraph={makeGraph} selectedDate={selectedDate} title={title} />}
        {id === 'avgtemp' && <TempMAM data={data} lineColor={color} startIdx={startIdx} endIdx={endIdx} fillGraph={fillGraph} tension={tension} makeGraph={makeGraph} selectedDate={selectedDate} title={title} />}
        {id !== 'DLI' && id !== 'GDD' && id !== 'avgtemp' && <p>Please provide a valid id.</p>}  
